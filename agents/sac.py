...
class SACAgent(BaseAgent):
    ...
    def update(self):
        ...
        return policy_loss.item(), (q_1_loss.item() + q_2_loss.item()) / 2, {"alpha": self.alpha}

    def _soft_update_networks(self, source, target):
        for target_param, param in zip(target.parameters(), source.parameters()):
            target_param.data.copy_(self.tau * param.data + (1.0 - self.tau) * target_param.data), {"alpha": self.alpha}
