...
def make_carracing_env(
    ...
    """
    Factory function to create a fully wrapped CarRacing environment.

    Args:
        render_mode: Gymnasium render mode ('human', 'rgb_array', None)
        preprocess: Apply preprocessing (grayscale, resize, normalize)
        frame_stack: Number of frames to stack (0 to disable)
        frame_skip: Action repeat count (1 = no repeat)
        use_throttle: Use 2D action space with throttle (else 3D)
        reward_scale: Reward scaling factor
        skip_zoom: Steps to skip at episode start
        patience: Steps without progress before truncation (None to disable)
        grayscale: Convert to grayscale
        resize: Target (height, width) for preprocessing
    ...
    # Action mapping (applied first, closest to environment)
    env = ActionMapWrapper(env, use_throttle=use_throttle)
    ...
    # Reward shaping
    if reward_scale != 1.0 or skip_zoom > 0 or patience is not None:
        env = RewardShapingWrapper(
            env,
            reward_scale=reward_scale,
            skip_zoom_steps=skip_zoom,
            patience=patience,
        )
    ...
    if preprocess:
        env = PreprocessWrapper(env, resize=resize, grayscale=grayscale)
