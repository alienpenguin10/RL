...
class PPOAgent:
    def __init__(self, env):
        ...
        self.policy = ActorCritic(
            input_shape=self.num_observations, output_shape=self.num_actions
        ).to(DEVICE)
    ...
    while total_steps < TOTAL_TIMESTEPS:
        ...
        for step in range(HORIZON):
            ...
            with torch.no_grad():
                ...
                action_tensor, log_prob_tensor, value_tensor = agent.policy.get_action(
                    state.unsqueeze(0)
                )
            ...
            if "episode" in info:
                print(
                    f"Global Step: {total_steps}, Episode Return: {info['episode']['r']}, Length: {info['episode']['l']}"
                )
