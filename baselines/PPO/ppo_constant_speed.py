...
from env.wrappers import FrameStackWrapper, PreprocessWrapper
...
class ActorCritic(nn.Module):
    ...
    def get_action(self, state):
        ...
        steer_log_prob = (
            steer_dist.log_prob(steer_action).sum(1).cpu().numpy().flatten()
        )
...
class PPOAgent:
    def __init__(self, env):
        ...
        self.policy = ActorCritic(num_frames=NUM_FRAMES, output_shape=1).to(
            DEVICE
        )  # Only steer
        self.optimizer = Adam(self.policy.parameters(), lr=LEARNING_RATE)
        self.lr_scheduler = LinearLR(
            self.optimizer, start_factor=1.0, end_factor=0.1, total_iters=NUM_UPDATES
        )
...
def train(env_name="CarRacing-v3", render_env=False, log_wandb=False):
    ...
    env = FrameStackWrapper(env, num_frames=NUM_FRAMES, skip_frames=SKIP_FRAMES)
    ...
    while total_steps < TOTAL_TIMESTEPS:
        ...
        for step in range(HORIZON):
            ...
            if terminated or truncated:
                print(
                    f"Episode {episode} | Episode steps {episode_steps} | Episode Reward={episode_reward:.2f}"
                )
                episode += 1
        ...
        returns, advantages = compute_gae(
            rewards, values, terminated, terminateds, next_value
        )
