...
def train(env_name='CarRacing-v3', model_path=None, eval_mode=False):
    ...
    if model_path is not None and os.path.exists(model_path):
        ...
        agent.lr_scheduler.load_state_dict(checkpoint['scheduler_state_dict'])
    ...
    while total_steps < TOTAL_TIMESTEPS:
        ...
        # Save model checkpoint every 100 updates
        if update_count % 100 == 0:
            checkpoint_path = os.path.join(models_dir, f'ppo_{env_name}_update_{update_count}.pt')
            torch.save({
                'update': update_count,
                'model_state_dict': agent.policy.state_dict(),
                'optimizer_state_dict': agent.optimizer.state_dict(),
                'scheduler_state_dict': agent.lr_scheduler.state_dict(),
