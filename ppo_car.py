...
class ActorCritic(nn.Module):
    def __init__(self, input_shape, output_shape):
        ...
        self.critic = nn.Sequential(
            layer_init(nn.Conv2d(in_channels=num_frames, out_channels=16, kernel_size=7, stride=4, padding=(8,2))), # (B,16,24,24)
            nn.ReLU(),
            layer_init(nn.MaxPool2d(kernel_size=2, stride=2)), # (B,16,12,12)
            layer_init(nn.Conv2d(in_channels=16, out_channels=32, kernel_size=5, stride=1, padding=1)), # (B,32,10,10)
            nn.ReLU(),
            layer_init(nn.Conv2d(in_channels=32, out_channels=64, kernel_size=3, stride=1)), # (B,64,8,8)
            nn.ReLU(),
            layer_init(nn.Conv2d(in_channels=64, out_channels=128, kernel_size=3, stride=1)), # (B,128,6,6)
            nn.ReLU(),
            layer_init(nn.Conv2d(in_channels=128, out_channels=256, kernel_size=3, stride=1)), # (B,256,4,4)
            nn.ReLU(),
            layer_init(nn.Linear(256*4*4, 512)),
            nn.Tanh(),
            layer_init(nn.Linear(512, 3), std=1.0),
        )
